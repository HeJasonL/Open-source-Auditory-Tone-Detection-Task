...
expName = 'headphone_calibration_check'  # from the Builder filename that created this script
expInfo = {'participant': '', 'session': '', 'level': '0.8912509381337451'}
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\W2032175\\Documents\\GitHub\\Open-source-Auditory-Tone-Detection-Task\\Headphone Calibration\\headphone_calibration_check_lastrun.py',
...
title_text = visual.TextStim(win=win, name='title_text',
    text='Headphone Calibration Check',
    font='Arial',
...
text_2 = visual.TextStim(win=win, name='text_2',
    text='Press space if the value presented on the screen is the value presented on your dB SPL meter',
    font='Arial',
...
text = visual.TextStim(win=win, name='text',
    ...
    languageStyle='LTR',
    depth=-2.0);
sound_1 = sound.Sound('pure_tone_1000Hz.wav', secs=-1, stereo=True, hamming=True,
    name='sound_1')
...
startComponents = [press_space, title_text, key_resp]
...
trials = data.TrialHandler(nReps=1, method='sequential', 
    ...
    trialList=data.importConditions('desired_db.xlsx'),
...
for thisTrial in trials:
    ...
    #estimate psychopy value to produce given dB
    #level is the value inserted at the beginning, which should have come from headphone_calibation.psyexp
    #desired dB are the list of desired dB output in the calibration.xlsx file
    
    
    psychopy_input_value = ((level)/(10**(117/20))*(10**(desired_db/20)))
    
    #print
    print('level', level)
    print('desired_db', desired_db)
    print('psychopy_input_value', psychopy_input_value)
    
    
    text.setText(desired_db)
    sound_1.setSound('pure_tone_1000Hz.wav', secs=20, hamming=True)
    sound_1.setVolume(psychopy_input_value, log=False)
    key_resp_2.keys = []
    key_resp_2.rt = []
    _key_resp_2_allKeys = []
    # keep track of which components have finished
    trialComponents = [text_2, text, sound_1, key_resp_2]
    ...
    while continueRoutine:
        ...
        if sound_1.status == STARTED:
            ...
            if tThisFlipGlobal > sound_1.tStartRefresh + 20-frameTolerance:
