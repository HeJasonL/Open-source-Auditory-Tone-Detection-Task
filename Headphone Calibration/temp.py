...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\k2032175\\Documents\\temp.py',
...
for thisTrial in trials:
    ...
    key_resp.rt = []
    _key_resp_allKeys = []
    text_2.setText('Does your dB SPL meter read 113 db?'

level)
    # keep track of which components have finished
    trialComponents = [sound_1, key_resp, text_2]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if key_resp.status == STARTED and not waitOnFlip:
            theseKeys = key_resp.getKeys(keyList=['d', 'u'], waitRelease=False)
            _key_resp_allKeys.extend(theseKeys)
            if len(_key_resp_allKeys):
                key_resp.keys = _key_resp_allKeys[-1].name  # just the last key pressed
                key_resp.rt = _key_resp_allKeys[-1].rt
                # was this correct?
                if (key_resp.keys == str('d')) or (key_resp.keys == 'd'):
                    key_resp.corr = 1
                else:
                    key_resp.corr = 0
                # a response ends the routine
    ...
    trials.addOtherData('text_2.started', text_2.tStartRefresh)
    trials.addOtherData('text_2.stopped', text_2.tStopRefresh)
    thisExp.nextEntry()
