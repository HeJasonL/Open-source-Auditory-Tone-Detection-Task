...
let expName = 'temp';  // from the Builder filename that created this script
let expInfo = {'participant': '', 'input': '0.2', 'session': '001'};
...
function trialRoutineEachFrame(trials) {
  return function () {
    ...
    // update/draw components on each frame
    // start/stop sound_1
    sound_1.setVolume(input_value, log=False)
    ...
    if (sound_1.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      if (30 > 0.5) {  sound_1.stop();  // stop the sound (if longer than duration)
    ...
    if (key_resp.status === PsychoJS.Status.STARTED) {
      let theseKeys = key_resp.getKeys({keyList: ['d', 'u'], waitRelease: false});
