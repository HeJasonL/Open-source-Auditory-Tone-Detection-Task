...
expName = 'longToneDetection_BETA001'  # from the Builder filename that created this script
expInfo = {'participant': '', 'session': '001', 'Tone Length': ['Short', 'Long'], 'Tone Frequency': '1000', 'Condition': ['Temporal Centre', 'Backward Masking', 'Simultaneous Onset', 'Simultaneous Delay', 'Forward Masking']}
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/jasonhe/Dropbox/Documents/GitHub/Open-source Auditory Tone Detection (OSATD) Task/Auditory Tone Detection Task/longToneDetection_BETA001_lastrun.py',
...
##Developer section:
##I'm using this as an alternative to expInfo for the time being (i.e., for the time I am using BUILDER view)

#Adjustments made based on Tone
if expInfo['Tone Length'] == 'Long':
    toneDuration = 0.2 #200ms
if expInfo['Tone Length'] == 'Short':
        toneDuration = 0.02 #20ms

#Adjustments made based on Condition 
#These are made relative to the Bandpass Noise
#Bandpass Noise 1 starts: 1 AND finishes: 1.3
#Bandpass Noise 2 starts: 2.1 AND finishes: 2.4

# Long Tone Condition
if expInfo['Condition'] == 'Temporal Centre':
    probe1Onset = 1.05 
    probe2Onset = 2.15

# Short Tone Conditions 
if expInfo['Condition'] == 'Backward Masking': 
    probe1Onset = 0.98 #1 (bandpassNoise1 onset) - .02
    probe2Onset = 2.08 #2.1 (bandpassNoise2 onset) - .02

if expInfo['Condition'] == 'Simultaneous Onset':
    probe1Onset = 1
    probe2Onset = 2.1

if expInfo['Condition']  == 'Simultaneous Delay':
    probe1Onset = 1.2 #1 - .2
    probe2Onset = 2.3 #2.1 + .2

if expInfo['Condition']  == 'Forward Masking':
    probe1Onset = 1.30 #1.3 is the offset of bandpassNoise1
    probe2Onset = 2.40 #2.4 is the offset of bandpassNoise2
...
questionText = visual.TextStim(win=win, name='questionText',
    text='In which interval ("1" or "2") do you think you heard the tone? \n\nRespond using the number pad. If you are unsure, just pick the option you are most confident with.',
    font='Arial',
...
for thisTrial in trials:
    ...
    # update component parameters for each repeat
    if condition == 1:
        probe1_toneDuration = toneDuration 
        probe2_toneDuration = 0
    
    elif condition == 2:
        probe1_toneDuration = 0
        probe2_toneDuration = toneDuration
    probeTone1.setSound('1000', secs=probe1_toneDuration)
    probeTone1.setVolume(toneVolume, log=False)
    probeTone2.setSound('1000', secs=probe2_toneDuration)
    ...
    while continueRoutine:
        ...
        if t >= probe1Onset and probeTone1.status == NOT_STARTED:
            # keep track of start time/frame for later
        ...
        frameRemains = probe1Onset + probe1_toneDuration- win.monitorFramePeriod * 0.75  # most of one frame period left
        if probeTone1.status == STARTED and t >= frameRemains:
            ...
            if probe1_toneDuration > 0.5:  # don't force-stop brief sounds
                probeTone1.stop()
        ...
        if t >= probe2Onset and probeTone2.status == NOT_STARTED:
            # keep track of start time/frame for later
        ...
        frameRemains = probe2Onset + probe2_toneDuration- win.monitorFramePeriod * 0.75  # most of one frame period left
        if probeTone2.status == STARTED and t >= frameRemains:
            ...
            if probe2_toneDuration > 0.5:  # don't force-stop brief sounds
                probeTone2.stop()
    ...
    # update component parameters for each repeat
    if questionResponse.keys[0] == str(condition):
       #Adjust tone volume
       toneVolume = toneVolume - 0.05 #tone volume goes down 2 dB for every correct response
       print('Tone Volume is now:', toneVolume, type(toneVolume))
    
    elif questionResponse.keys[0] != str(condition):
       #Adjust tone volume
       toneVolume = toneVolume + 0.05 #tone volume goes down by 2dB for every correct response
       print('Tone Volume is now:', toneVolume)
    
    if toneVolume < .5 - .45: #if tone volume falls below .05 (which can't be represented exactly in python for reasons that currently escape me
        print('Tone Volume now below zero') 
        toneVolume = .5 - .45 #set the tone to be at .05 (so it can't fall below)
    
    elif toneVolume == 0:
            print('Tone Volume has been set to zero')
    
    elif toneVolume > 0:
            print('Tone Volume still above zero')
