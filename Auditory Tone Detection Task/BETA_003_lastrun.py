...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/jasonhe/Dropbox/Documents/Projects/Auditory detection tasks AIMS Project/Auditory Tone Detection Task/BETA_003_lastrun.py',
...
#Initial color intensity of the feedback stimulus (used for debugging)
intensity = .5

#Initial tone volume (in psychopy units)
toneVolume = 0.5
Long_Tone1 = sound.Sound('1000', secs=-1, stereo=True)
...
ISI = visual.TextStim(win=win, name='ISI',
    ...
    languageStyle='LTR',
    depth=-3.0);
Long_Tone2 = sound.Sound('1000', secs=-1, stereo=True)
...
for thisTrial in trials:
    ...
    continueRoutine = True
    # update component parameters for each repeat
    Long_Tone1.setSound('1000', secs=startProbe1)
    Long_Tone1.setVolume(toneVolume, log=False)
    Long_Tone2.setSound('1000', secs=startProbe2)
    Long_Tone2.setVolume(toneVolume, log=False)
    ...
    while continueRoutine:
        ...
        if Question_Response.status == STARTED:
            theseKeys = Question_Response.getKeys(keyList=['1', '2'], waitRelease=False)
            if len(theseKeys):
                theseKeys = theseKeys[0]  # at least one key was pressed
    ...
    # update component parameters for each repeat
    Feedback_text.setColor('white', colorSpace='rgb')
    Feedback_text.setText(toneVolume)
    if Question_Response.keys[0] == str(interval):
       #Attempt at adjusting tone volume 
       toneVolume = toneVolume - 0.05 #tone volume goes down 2 dB for every correct response
       print('Tone Volume is now:', toneVolume)
       
       #Adjusting feedback color for debugging
       intensity = intensity - 0.05
       Feedback_text.setColor([0,intensity,0], colorSpace='rgb') #GREEN
    
    
    elif Question_Response.keys[0] != str(interval):
       #Attempt at adjusting tone volume 
       ...
       #Adjusting feedback color for debugging
       intensity = intensity + 0.05 
       Feedback_text.setColor([intensity,0,0], colorSpace='rgb') #RED 
